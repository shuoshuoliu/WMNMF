...
differror = options.error;            % set 1e-6
maxIter = options.maxIter;            % set 300
nRepeat = options.nRepeat;            % set 30
minIterOrig = options.minIter;        % set 50
minIter = minIterOrig-1;              % here 49
meanFitRatio = options.meanFitRatio;  % set 0.1
...
bSuccess.bSuccess = 1;
selectInit = 1;
if isempty(U)  % if U not provided, then randomly generate
    U = abs(rand(mFea,k));
    V = abs(rand(nSmp,k));
else
    nRepeat = 1; % the case in the paper
end
...
% the above calculate the initials
% the below update u,v 
...
function [obj, dV] = CalculateObj(X, U, V, L, alpha, deltaVU, dVordU)
    ...
    % the above are not set generally

    dV = [];
    maxM = 62500000;
    [mFea, nSmp] = size(X);
    mn = numel(X); % calculate the number of array elements
